...
func ListDictItem(ctx context.Context, query DictItemQuery) ([]DictItem, int64, error) {
	var list []DictItem
	var total int64
	d := repository.DB(ctx).Model(&DictItem{}).Where("dict_id = ?", query.DictId)
	if query.ItemKey != "" {
...
func UpdateDictItem(ctx context.Context, item DictItem) error {
	return repository.DB(ctx).Model(&item).Where("id = ?", item.Id).Updates(&item).Error
}

func UpdateDict(ctx context.Context, dict Dict) error {
	return repository.DB(ctx).Model(&dict).Where("id = ?", dict.Id).Updates(&dict).Error
}
